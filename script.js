...
$('.content__table-bid').on('click',function() {
		if (!$(this).hasClass('content__table-bid_check')) {
			$(this).parent().children('.content__table-bid_check').removeClass('content__table-bid_check').addClass('content__table-bid_cancel');
			$(this).addClass('content__table-bid_check').removeClass('content__table-bid_cancel');
		}
})

$('.content__table-actions-one').on('click',function() {
	var action = $(this).attr('data-action');
	$(this).parents('.content__table-actions').children('span').text(action);
})

$('.content__table-all-check').on('click',function(){
	if ($(this).is(':checked')){
		$('.content__table-small input:checkbox').prop('checked', true);
		var count = $('.content__table-one-check:checked').length;
		$('.content__table-choice span').text(count);
	} else {
		$('.content__table-small input:checkbox').prop('checked', false);
		$('.content__table-choice span').text('0');
	}
});

$('.content__table-one-check').on('click',function(){
	var count = $('.content__table-one-check:checked').length;
	$('.content__table-choice span').text(count);
})
...
$('.contests__class').on('click',function(){
	...
	if (!$(this).hasClass('contests__class_active')) {
		...
		$(this).parents('.main-content__inner').find('.content__tab').hide(0).removeClass('content__tab_active');
		$('#'+tab).show(0).addClass('content__tab_active');
		console.log(tab);
	}
})
